...
public class Main {
	...
	public static void main(String[] args) {
		...
		Parking exmp = parseFileToParking(args[0]);

		// Verifie la possibilite d'une solution
		String impossibleMessage = exmp.isImpossible();
		if (impossibleMessage != "") {	//Solution impossible
			...
			Parking solution = solveParking(exmp);
	...
	static Parking solveParking(Parking exmp) {
		/*
		 * Retourne la meilleure solution au problème du Parking ayant pour racine exmp.
		...
		prevParkings.add(exmp); //Premier niveau: racine

		//Si le Goal Car est déjà libre:
		Parking alreadySolved = exmp.isSolved();
	...
	static void printSteps(Stack<Parking> path) {
		/*
		 * Affiche la résolution du problème, à partir du Stack contenant chaque etape menant a la
		 * solution.
		 */
		...
		System.out.println(String.format("\n Une facon de sortir du Parking en %d mouvements a ete trouvee.", path.size()));
		...
		while (!path.isEmpty()) {
			...
			if (prevMovedCar != thisMovedCar) {
				if (thisMovedCar == 0) {
					System.out.println(String.format("\n Deplacements voiture Goal :", thisMovedCar));
				}
				else {
					System.out.println(String.format("\n Deplacements car %d :", thisMovedCar));
				}
	...
	static Parking parseFileToParking(String filename) {
		...
		try {
			...
			try {
				...
				buffrd.close();
				filerd.close();
				return new Parking(width, height, exitX, exitY, carCoordinates); // Fichier correct: retourner le parking
			}
